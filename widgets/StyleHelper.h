...
namespace StyleHelper {
    ...
    inline QString getPrimaryColor() {
        switch (currentTheme) {
            case Theme::Dark:     return "#75B5FF"; // 더 밝은 블루 (다크 모드 가독성)
    ...
    inline QString getBgColor() {
        switch (currentTheme) {
            case Theme::Dark:     return "#1E1E1E"; // 더 깊은 배경색
    ...
    inline QString getTextColor() {
        switch (currentTheme) {
            case Theme::Dark:     return "#F5F5F5"; // 아주 밝은 그레이
    ...
    inline QString getFormLabelStyle() {
        QString color;
        if (currentTheme == Theme::Dark) color = "#BBBBBB"; // 다크 모드용 부드러운 라벨색
        else if (currentTheme == Theme::DeepBlue) color = "#1967D2";
        else color = "#666";
    ...
    inline QString getListWidgetStyle() {
        QString border = (currentTheme == Theme::Dark) ? "#333" : (currentTheme == Theme::DeepBlue ? "#ADCCFB" : "#DDD");
        QString bg = (currentTheme == Theme::Dark) ? "#252525" : (currentTheme == Theme::DeepBlue ? "#F1F6FF" : "white");
    ...
    inline QString getCommonInputStyle() {
        ...
        QString bg = (currentTheme == Theme::Dark) ? "#2D2D2D" : "#FDFDFD";
        QString text = getTextColor();
        QString border = (currentTheme == Theme::Dark) ? "#444" : (currentTheme == Theme::DeepBlue ? "#ADCCFB" : "#DDD");
        ...
        return QString(
            ...
            "  border-left: 1px solid %1; border-top-right-radius: 6px; border-bottom-right-radius: 6px; background: %6;"
            "}"
            "QComboBox::down-arrow, QDateTimeEdit::down-arrow {"
            "  width: 0; height: 0; border-left: 4px solid transparent; border-right: 4px solid transparent; border-top: 5px solid %7;"
            "}"
        ).arg(border, bg, text, primary, (currentTheme == Theme::Dark ? "#333" : "white"), (currentTheme == Theme::Dark ? "#383838" : "#F5F5F5"), (currentTheme == Theme::Dark ? "#CCC" : "#888"));
    }

    inline QString getScrollbarStyle() {
        QString handle = (currentTheme == Theme::Dark) ? "#555" : (currentTheme == Theme::DeepBlue ? "#AECBFA" : "#E0E0E0");
        QString hover = (currentTheme == Theme::Dark) ? "#777" : (currentTheme == Theme::DeepBlue ? "#8AB4F8" : "#BDBDBD");
        return QString(
    ...
    inline QString getCalendarPopupStyle() {
        QString text = getTextColor();
        QString primary = getPrimaryColor();
        QString bg = (currentTheme == Theme::Dark) ? "#2D2D2D" : getBgColor();
        return QString(
            ...
            "QCalendarWidget QToolButton:hover { background-color: %3; }"
            "QCalendarWidget QMenu { background-color: %1; color: %2; border: 1px solid #444; }"
            "QCalendarWidget QSpinBox { color: %2; background-color: %1; border: 1px solid #444; padding-right: 15px; }"
            "QCalendarWidget QAbstractItemView:enabled { color: %2; selection-background-color: %4; selection-color: white; }"
        ).arg(bg, text, (currentTheme == Theme::Dark ? "#444" : "#EEE"), primary);
    }
    ...
    inline QString getItemBaseStyle(const QString& catColor) {
        QString bg = (currentTheme == Theme::Dark) ? "#2D2D2D" : "white";
        QString border = (currentTheme == Theme::Dark) ? "#444" : (currentTheme == Theme::DeepBlue ? "#D2E3FC" : "#EEE");
        return QString("#itemWidget { background-color: %1; border-left: 5px solid %2; border-radius: 6px; border-top: 1px solid %3; border-right: 1px solid %3; border-bottom: 1px solid %3; }").arg(bg, catColor, border);
    }

    inline QString getItemHoverStyle(const QString& catColor) {
        QString bg = (currentTheme == Theme::Dark) ? "#383838" : (currentTheme == Theme::DeepBlue ? "#F1F6FF" : "#F5F9FF");
